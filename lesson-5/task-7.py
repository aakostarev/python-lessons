...
with open('task-7.json', 'w', encoding='utf-8') as json_file:
    with open('task-7.txt', encoding='utf-8') as text_file:
        profit = {element.split()[0]: int(element.split()[2]) - int(element.split()[3]) for element in text_file}
        aver = [profit, {'average profit': round(sum([int(i) for i in profit.values() if int(i) > 0]) /
                                                     len([int(i) for i in profit.values() if int(i) > 0]))}]
    json.dump(aver, json_file, ensure_ascii=False, indent=4)