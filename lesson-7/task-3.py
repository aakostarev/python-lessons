...
# Сложение. Объединение двух клеток. При этом число ячеек общей клетки должно равняться сумме ячеек и
# сходных двух клеток.
...
class Cell:
    ...
    def __sub__(self, other):
        return self.nums - other.nums if self.nums - other.nums > 0 \
            else 'Ячеек в первой клетке не больше чем во второй, нельзя вычесть'

    def __mul__(self, other):
        return f'Произведение ячеек: {self.nums * other.nums}'

    def __floordiv__(self, other):
        return f'Целочисленное частное ячеек: {self.nums // other.nums}'